...
def getData(pathToData, trainFraction=None, sep='[^-?\d+\.?\d*?e\d*]'):
	...
	sep : delimiter between entries in data
	    ...
	    flexibility for possible delimiters
	"""
	data, dataROWS, dataCOLS = readFile(pathToData, nrows=None, xcolmin=1, xcolmax=None, sep='[^-?\d+\.?\d*?e\d*]')

	if not trainFraction:
		testData = {'data': data[:, 1:], 'labels': data[:, 0]}
		return testData
	...
	trainData = {'data': data[:trCutoff, 1:], 'labels': data[:trCutoff, 0]}
	valData = {'data': data[trCutoff:, 1:], 'labels': data[trCutoff:, 0]}
