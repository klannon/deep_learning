...
low = float(raw_input("Enter the lower bound for the y-axis: ").rstrip())
high = float(raw_input("Enter the upper bound for the y-axis: ").rstrip())

for nF, filename in enumerate(args.files):
    tr = ROOT.TGraph()
    te = ROOT.TGraph()
    g_name = raw_input("Enter name for graph {}: ".format(nF+1))
    tr.SetPoint(1, 0, 0.0)
    te.SetPoint(1, 0, 0.0)
    tr.SetTitle("Train: {}".format(g_name))
    te.SetTitle("Test: {}".format(g_name))
    try:
...
mg.Draw("AC")
mg.SetTitle(title)
mg.GetXaxis().SetTitle(x_axis)
mg.GetYaxis().SetTitle(y_axis)
mg.SetMinimum(low)
mg.SetMaximum(high)
